...
class BaseDocument {
	constructor(DB) {
        checkParamDB(DB, this.constructor.name + ' constructor');
    ...
    static collectionName() {
        // not good enough; will require overriding
    ...
    validate() {
        ...
        _.keys(that._schema).forEach(function(key) {
            ...
            if (!isValidType(that._camoDB, value, that._schema[key].type)) {
    ...
    static create(DB, data) {
        checkParamDB(DB, this.name + '.create');
        this.createIndexes(DB);

        if (typeof(data) !== 'undefined') {
            return this._fromData(DB, data);
        }

        return this._instantiate(DB);
    }

    static createIndexes(DB) { }

    static _instantiate(DB) {
        var instance = new this(DB);
    ...
    static _fromData(DB, datas) {
        ...
        datas.forEach(function(d) {
            var instance = that._instantiate(DB);
            _.keys(d).forEach(function(key) {
                ...
                if (key in instance._schema) {
                    ...
                    if (type.documentClass && type.documentClass() === 'embedded') {
                        ...
                        instance[key] = type._fromData(DB, value);
                        ...
                        value.forEach(function(v, i) {
                            instance[key][i] = type[0]._fromData(DB, v);
                        });
                    } else {
    ...
    populate() {
        return BaseDocument.populate(this._camoDB, this);
    }

    // TODO : EMBEDDED
    // 
    static populate(DB, docs, fields) {
        ...
        _.keys(ids).forEach(function(key) {
            ...
            var p = type.find(DB, { '_id': { $in: keyIds } }, { populate: false })
